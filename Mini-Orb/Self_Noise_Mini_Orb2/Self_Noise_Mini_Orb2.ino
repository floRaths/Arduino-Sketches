...
DEFINE_GRADIENT_PALETTE( pit ) {
  0, 3, 3, 3,
  22,   100, 0,   103, // purple
  33,   101, 48,193, // light purple
  44, 75,0,130, // indigo
  55, 128,0,128, // magenta
  66, 153,50,204, // dark orchid
  77, 255,0,255, //fuchsia
  88, 3, 3, 3,
  110, 199,21,133, // medium violetred
  132, 255,20,147, // deeppink
  143, 220,20,60, // crimson
  154, 25,25,112, // midnight blue
  165, 72,61,139, // darkslate blue
  176,   94,  100,204,// light blue 
  187, 3, 3, 3,
  109,  0,96,99,   //  tuquoise
  220,  72,209,204,// light turquoise
  231, 0,139,139,  // dark cyan
  242, 32,178,170, //lightseagreeen 
  250,  0,100,100, // darker cyan
  255,  0,128,128  // teal  
};


DEFINE_GRADIENT_PALETTE( pit1 ) {
  0,   0,100,100,
  255, 0,100,100
};
...
void setup() {
  ...
  //LEDS.addLeds<CHIPSET, LED_PIN, COLOR_ORDER, DATA_RATE_MHZ(12)>(leds, NUM_LEDS).setCorrection(TypicalSMD5050);
  LEDS.addLeds<CHIPSET, LED_PIN, COLOR_ORDER>(leds, NUM_LEDS).setCorrection(TypicalSMD5050);
...
void noise_noise1() {
  ...
  z[0] += 1 + ((noise[5][5][ kMatrixHeight-1]) / 4);
  ...
  scale_x[0] = 4000 + (noise[5][5][CentreY] * 16);
  scale_y[0] = 4000 + (noise[5][kMatrixWidth-1][CentreY] * 16);
